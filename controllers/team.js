...
exports.postTeam = function(req, res) {
  ...
  Team.forge(team).save()
    .then(function(createdTeam) {
      team.stats = req.body.stats || [];
      team.members = req.body.members || [];

      var teamId = createdTeam.id;
      var all = [];
      all.push(
        new Team({id: teamId}).setStatsAndSave(team.stats)
      );

      all.push(
        new Team({id: teamId}).setMembersAndSave(team.members)
      );
      return Promise.all(all).then(function(){return createdTeam});
    })
    .then(function(team) {
...
exports.putTeam = function(req, res) {
  ...
  all.push(
    new Team({id: teamId}).setStatsAndSave(newObj.stats)
  );
